...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        Posting p3 = new PostingImpl(3L,"El seis esquinas", "Home swett home", "Moldes 2536, Mendoza", "www.foto.jpg", true, new FeatureImpl(235.0, 1, 4, 2, 2), true, true, true, false, PublicationType.CLASIFFIED, OperationType.ALQUILER,CategoryListing.PARTICULAR, new PriceImpl(450000.0, 30000.0, 10.0), new PublisherImpl());
...
//        List<Posting> postings = Util.getPostings();
        //Util.sortPostingsByPublicationType(postings);
//        Util.sortPostingsById(postings);
        //Util.sortPostingsByBathroom(postings);
 //       postings.forEach(x -> System.out.println(x.toString()));

        Filter filter = new FilterImpl(OperationType.ALQUILER,true, true, true, false, PublicationType.CLASIFFIED, 1, 3, 1, 3);
        MatchPosting mp = new MatchPosting();

        List<Posting> filterList = mp.postings(filter);
